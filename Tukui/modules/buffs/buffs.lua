...
local neutral = [[Interface\Icons\Inv_misc_questionmark]]
...
local Skin = function(self)
	...
	if(not proxy) then
		...
		-- used to update icon if pandaren with no faction
		if T.myfaction == "Neutral" then
			self:RegisterEvent("NEUTRAL_FACTION_SELECT_RESULT")
			self:SetScript("OnEvent", function(self, event)
				if event ~= "NEUTRAL_FACTION_SELECT_RESULT" then return end
				T.myfaction = UnitFactionGroup("player")
				if T.myfaction == "Horde" then
					Overlay:SetTexture(horde)
				else
					Overlay:SetTexture(alliance)
				end
