...
for _, menu in pairs(UnitPopupMenus) do
	for index = #menu, 1, -1 do
		if menu[index] == "SET_FOCUS" or menu[index] == "CLEAR_FOCUS" or menu[index] == "MOVE_PLAYER_FRAME" or menu[index] == "MOVE_TARGET_FRAME" then
...
if C.unitframes.raid == true then
	...
	if C.unitframes.showraidpets then
		local function GetPetAttributes()
			...
			"maxColumns", 8, 
			"point", point,
			"unitsPerColumn", C.unitframes.raidunitsperrow,
			"columnSpacing", T.Scale(3),
			"columnAnchorPoint", columnAnchorPoint,
			"yOffset", T.Scale(-3),
			"xOffset", T.Scale(3),
