...
local Update = function(self, event)
	...
	local isAssistant = UnitInRaid(unit) and (WoW5 and UnitIsGroupAssistant(unit) or UnitIsRaidOfficer(unit)) or (not WoW5 and UnitIsGroupLeader(unit) or not WoW5 and UnitIsPartyLeader(unit))
	if(isAssistant) then
		assistant:Show()
	else
		assistant:Hide()
